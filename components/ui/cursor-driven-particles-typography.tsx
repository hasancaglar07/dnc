...
export function CursorDrivenParticleTypography({
  ...
  useEffect(() => {
    ...
    const init = () => {
      ...
      const step = Math.max(2, Math.floor(particleDensity * dpr));
      const maxParticles = 3000; // Limit total particles for performance
      let particleCount = 0;
    ...
    const animate = () => {
      ...
      ctx.fillStyle = color || window.getComputedStyle(containerRef.current!).color || "#000000";
